...
- 结论: **`make test` 与 `make verify MODULE=puncturing` 均退出码 0** (快门全绿 + mutation 6/6=100%)。
...
## 6. mutation kill rate — 6/6 = 100% (≥90% 达标)

`make verify MODULE=puncturing` exit 0。`scripts/mutate.py` 重构为按模块字典
(`MUTATIONS_BY_MODULE`), 为 puncturing 新增 6 个变异, 均成功注入且被现有 ref/compare/sva/selfcheck 杀死:

| 变异 | 注入的错误 | 杀死者 |
|---|---|---|
| pat15_tap | Pat15of16 翻 bit0 (1→0) | compare (逐bit) |
| adv_plus1 | 相位推进 +2 → +1 | compare |
| keep_misidx | keep_a0 读 eff_phase+1 的模式位 | compare |
| wrap_off1 | 回绕 `>=` → `>` (off-by-one) | compare |
| no_seqreset | seq_start 拍 eff_phase 不置 0 | selfcheck + compare |
| break_pass | rate=1/2 模式向量 bit0→0 (破坏透传) | SVA A1 + compare |

**无存活变异** → 未发现验证盲区, 无需补强 TB/激励。fec_encoder 原 6 条变异语义未动 (仍 6/6 杀死),
mutate.py 对未定义变异组的模块给明确报错 (非静默放过)。
...
- [x] mutation kill rate 6/6 = 100% (≥90%)  [x] make verify 整体 exit 0
